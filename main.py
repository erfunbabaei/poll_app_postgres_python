...
def show_poll_votes():
    ...
    try:
        for option, votes in zip(options, votes_per_option):
            ...
            print(f"{option.option_text} for {votes} ({percentage:.2f}% of total)")
...
def _print_votes_for_options(options: list[Option]):
    for option in options:
        ...
        for vote in option.votes:
            local_date = datetime.datetime.fromtimestamp(vote[2], pytz.timezone("Asia/Tehran")).strftime("%Y-%m-%d %H:%M")
...
def menu():
    with get_connection() as connection:
        database.create_poll_schema(connection)
